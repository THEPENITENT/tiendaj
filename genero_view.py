# genero_view.py — Pantalla CRUD de la tabla Genero (abuela, sin FK)
import customtkinter as ctk
from tkinter import messagebox
from colores import *
from widgets import (make_topbar, make_entry, make_boton,
...
class GeneroView:
    """
    Pantalla de mantenimiento de la tabla Genero.
    - idGenero:     VARCHAR(5)  → máx 5 chars
    - NombreGenero: VARCHAR(50) → máx 50 chars
    """
    ...
    def _construir(self):
        ...
        ttl.pack(side="left")
        ctk.CTkLabel(ttl, text="Mantenimiento de Géneros",
    ...
    def _form(self, parent):
        ...
        form = ctk.CTkFrame(left, fg_color=BG_CARD, corner_radius=12,
                            border_width=1, border_color=BORDER, width=280)
        form.pack(fill="x")
        form.columnconfigure(0, weight=1)

        ctk.CTkLabel(form, text="ID GÉNERO  (máx. 5)",
                     font=("Segoe UI", 10, "bold"),
                     text_color=GRAY_TEXT).grid(row=0, column=0, sticky="w", padx=18, pady=(16, 3))
        self.ent_id = make_entry(self.ventana, form, "Ej. ACC", max_chars=5)
        ...
        ctk.CTkLabel(form, text="NOMBRE GÉNERO  (máx. 50)",
                     font=("Segoe UI", 10, "bold"),
                     text_color=GRAY_TEXT).grid(row=2, column=0, sticky="w", padx=18, pady=(14, 3))
        self.ent_nombre = make_entry(self.ventana, form, "Ej. Acción", max_chars=50)
        ...
        self.lbl_status = make_status(left)
    ...
    def _buscar(self):
        ...
        try:
            ...
            if row:
                ...
                self._s(f"✔  Género '{id_}' encontrado.", CYAN)
            else:
                self.ent_nombre.delete(0, "end")
    ...
    def _alta(self):
        ...
        try:
            ...
            self.dao.insertar(id_, nom)
            self._s(f"✔  Género '{id_}' guardado.", GREEN)
    ...
    def _cambios(self):
        ...
        try:
            ...
            self.dao.actualizar(id_, nom)
            self._s(f"✔  Género '{id_}' actualizado.", GREEN)
            self._refrescar()
        except Exception as e:
            self._s(f"✖  {e}", RED)
    ...
    def _baja(self):
        id_ = self.ent_id.get().strip()
        if not id_:
            self._s("⚠  Escribe o selecciona un ID.", RED); self.ent_id.focus_set(); return
        try:
            if not self.dao.existe(id_):
                self._s(f"⚠  El ID '{id_}' no existe.", RED); return
            if self.dao.tiene_hijos(id_):
                self._s(f"✖  No se puede eliminar '{id_}': tiene registros asociados.", RED); return
            if not messagebox.askyesno("Confirmar baja",
                                       f"¿Eliminar el género '{id_}'?\nEsta acción no se puede deshacer."):
                return
            self.dao.eliminar(id_)
            self._s(f"✔  Género '{id_}' eliminado.", GREEN)
            self._limpiar(); self._refrescar()
        except Exception as e:
            self._s(f"✖  {e}", RED)
