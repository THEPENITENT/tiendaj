# clasificacion_view.py — CRUD de clasificacion (abuela, sin FK)
import customtkinter as ctk
from tkinter import messagebox
from colores import *
from widgets import make_form_panel, make_topbar, make_entry, make_boton, make_treeview, make_status, set_status
from clasificacion_dao import ClasificacionDAO


class ClasificacionView:
    """
    Pantalla CRUD de la tabla clasificacion.
    - idclasificacion:      VARCHAR(5)  máx 5, PK
    ...
    def __init__(self, ventana, usuario, puesto, cb_volver):
        ...
        self.dao       = ClasificacionDAO()
        self._construir()

    def _construir(self):
        self.ventana.geometry("860x660")
        ...
        ttl.pack(side="left")
        ctk.CTkLabel(ttl, text="Mantenimiento de Clasificaciones",
    ...
    def _form(self, parent):
        ...
        # ID — VARCHAR(5)
        ctk.CTkLabel(form, text="ID CLASIFICACIÓN  (máx. 5)",
                     font=("Segoe UI", 10, "bold"),
                     text_color=GRAY_TEXT).grid(row=0, column=0, sticky="w", padx=18, pady=(16, 3))
        self.ent_id = make_entry(self.ventana, form, "Ej. CL001", max_chars=5)
        ...
        ctk.CTkLabel(form, text="DESCRIPCIÓN  (máx. 50, NOT NULL)",
                     font=("Segoe UI", 10, "bold"),
                     text_color=GRAY_TEXT).grid(row=2, column=0, sticky="w", padx=18, pady=(14, 3))
        self.ent_desc = make_entry(self.ventana, form, "Nombre de la clasificación", max_chars=50)
        self.ent_desc.grid(row=3, column=0, sticky="ew", padx=18)

        ctk.CTkFrame(form, height=14, fg_color="transparent").grid(row=4, column=0)

        # Botón ver hijos
        ctk.CTkButton(
            form, text="🎮  Ver Videojuegos →", height=34,
            fg_color=CYAN, text_color=BG_DARK,
            font=("Segoe UI", 11, "bold"), corner_radius=8,
            command=self._ver_hijos
    ...
    def _sel_fila(self):
        ...
        self.ent_desc.delete(0, "end"); self.ent_desc.insert(0, v[1])
    ...
    def _buscar(self):
        ...
        try:
            ...
            if row:
                self.ent_desc.delete(0, "end")
                self.ent_desc.insert(0, row[1] or "")
                self._s(f"✔  Clasificación '{id_}' encontrada.", CYAN)
            else:
                self.ent_desc.delete(0, "end")
    ...
    def _alta(self):
        ...
        if not id_:
            self._s("⚠  El ID es obligatorio (NOT NULL).", RED); self.ent_id.focus_set(); return
        if not desc:
            self._s("⚠  La descripción es obligatoria (NOT NULL).", RED); self.ent_desc.focus_set(); return
        try:
            if self.dao.existe(id_):
                self._s(f"⚠  El ID '{id_}' ya existe. No se duplica la PK.", RED)
                self.ent_id.focus_set(); return
            self.dao.insertar(id_, desc)
            self._s(f"✔  Clasificación '{id_}' guardada.", GREEN)
    ...
    def _cambios(self):
        ...
        try:
            ...
            self.dao.actualizar(id_, desc)
            self._s(f"✔  Clasificación '{id_}' actualizada.", GREEN)
            self._refrescar()
        except Exception as e:
            self._s(f"✖  {e}", RED)
    ...
    def _baja(self):
        ...
        try:
            ...
            if self.dao.tiene_hijos(id_):
                self._s(f"✖  No se puede eliminar '{id_}': tiene videojuegos asociados (tiene hijos).", RED); return
            if not messagebox.askyesno("Confirmar baja",
                                       f"¿Eliminar clasificación '{id_}'?\nEsta acción no se puede deshacer."):
                return
            self.dao.eliminar(id_)
            self._s(f"✔  Clasificación '{id_}' eliminada.", GREEN)
            self._limpiar(); self._refrescar()
        except Exception as e:
            self._s(f"✖  {e}", RED)
    ...
    def _ver_hijos(self):
        id_ = self.ent_id.get().strip()
        if not id_:
            self._s("⚠  Selecciona una clasificación primero.", RED); return
        if not self.dao.existe(id_):
            self._s(f"⚠  El ID '{id_}' no existe en la BD.", RED); return
        from videojuego_view import VideojuegoView
        self.ventana._limpiar()
        VideojuegoView(self.ventana, self.usuario, self.puesto, self.cb_volver, id_)
